    ...
    var list=[];
     dd.httpRequest({
          ...
          success: (res) => {
            ...
            if(reList.length>0 && reList!=null){
              console.log("进来")
       ...
       dd.httpRequest({
          ...
          var _appStatusList=app.globalData.statusList;
           if(pur.status==_appStatusList.tobenegotiated){
            ...
            var status_pass=_appStatusList.pass;//-------------------*---------------------------------------最终结束改为 =4
            var status_refuse=_appStatusList.refuse;//-------------------*---------------------------------------最终结束改为 =4
            this.setData({
              statusPass:status_pass,
              statusRefuse:status_refuse
            })
          ...
          that.setData({
            ...
            "pur.approverProposal":pur.approverProposal//审核人意见
          })
...
//*------------------------------------------------------
  ...
  //审批人建议
  getApproverProposal(e){
    console.log("审批人建议"+JSON.stringify(e))
    ...
    //   targetObj:_ob
    // })
    console.log("负责人说明1："+JSON.stringify(this.data.approverProposal))
  ...
  submit(e){
    ...
    var _result="";
    console.log("11111",_id,re)

    if(re=="1"){
      //同意
      _result=app.globalData.statusList.pass;
    }
    if(re=="0"){
      //拒绝
      _result=app.globalData.statusList.refuse;
    }
    console.log("ddddd:",_id,_result,_approverProposal)
       dd.httpRequest({
          url: app.globalData.domain+'/negotiateding/saveToBeApproved',
          method: 'GET',
          data: {purId:_id,passOrNot:_result,approverProposal:_approverProposal},
          headers: {
