...
//*------------------------------------------------------
         ...
         // console.log("ccc",appList)
          ...
          //console.log("dddddd-----",_negoExplain)
  ...
  //查询审批人批注
    showComment(){
        ...
        var _taskId = that.data.taskId;
         dd.httpRequest({
          url: app.globalData.domain+'/ac/comments',
          method: 'GET',
          data: {taskId:_taskId},
          headers: {
            'eticket': app.globalData.eticket
          ...
          success: (res) => {
              ...
              if(reList.length>0 && reList!=null){
                for(var i=0;i<reList.length;i++){
                  if(reList[i]!=null && reList[i].length>0){
                    
                    var obj={
                    'userid':reList[i][0].userId,
                    'message':reList[i][0].message
                  }
  ...
  getApproverProposal(e){
    ...
    //   targetObj:_ob
    // })
   // console.log("负责人说明1："+JSON.stringify(this.data))
